...
class BaseVisualizer(object):
    def __init__(self, dataloader, save_path, logger, additional_args=None):
...
class FlowVisualizerEvents(BaseVisualizer):
    def __init__(self, dataloader, save_path, clamp_flow=True, logger=None, additional_args=None):
        super(FlowVisualizerEvents, self).__init__(dataloader, save_path, logger, additional_args)
        self.flow_scaling = 0
...
class EnaviVisualizerEvents(BaseVisualizer):
    def __init__(self, dataloader, save_path, logger=None, additional_args=None):
        super(EnaviVisualizerEvents, self).__init__(dataloader, save_path, logger, additional_args)
        # Create Visu folders for every sequence
        for name in self.additional_args['name_mapping']:
            path = os.path.join(self.visu_path, name)
            os.makedirs(os.path.join(self.visu_path, name))
            os.makedirs(os.path.join(self.visu_path, name, "flow"))
    ...
    def visualize_events(self, image, batch, batch_idx, sequence_name):
        ...
        loader_instance = self.dataloader.dataset.datasets[sequence_idx]
        h, w = loader_instance.get_image_width_height()
        events = loader_instance.event_slicer.get_events_no_idx(
            t_start_us=batch['timestamp'][batch_idx].item(),
            t_end_us=batch['timestamp'][batch_idx].item()+delta_t_us
        )
        p = events['p'].astype(numpy.int8)
        ...
        events_rectified = numpy.stack([t, x, y, p], axis=-1)
        event_image = events_to_event_image(
            event_sequence=events_rectified,
            height=h,
            width=w
        ...
        name_events = "{}.png".format(int(batch['file_index'][batch_idx].item()))
        out_path = os.path.join(self.visu_path, sequence_name, name_events)
        imageio.imsave(out_path, event_image.transpose(1,2,0))
        ...
        for batch_idx in range(len(batch['file_index'])):
            ...
            if batch['visualize'][batch_idx]:
                ...
                self.visualize_flow_colours(
                    ...
                    fix_scaling=None,
                    sub_folder=sequence_name,
                    custom_name= "{}.png".format(batch['file_index'][batch_idx])
                )
                # Visualize Events
                self.visualize_events(
                    image=None,
...
def visualize_optical_flow(flow, savepath=None, return_image=False, text=None, scaling=None, logger=None):
    ...
    mag = numpy.sqrt((flow[...,0]/div)**2+(flow[...,1]/div)**2)**0.5

    logger.write_line("{} {} {} {}".format(flow.min(), flow.max(), mag.min(), mag.max()), False)
