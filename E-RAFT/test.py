...
class Test(object):
    ...
    def __init__(self, model, config,
        ...
        if isinstance(self.additional_args, dict) and 'name_mapping_test' in self.additional_args.keys():
            ...
            visu_add_args = None
        self.visualizer = visualizer(data_loader, self.save_path, self.logger, additional_args=visu_add_args)
    ...
    def _test(self):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(self.data_loader):
                ...
                self.logger.write_line("Sample {}/{}".format(batch_idx + 1, len(self.data_loader)), False)
                self.logger.write_line("Inference Time {}".format(t_total), False)
