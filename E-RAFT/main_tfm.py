...
from loader.loader_enavi import *
...
def get_visualizer(args):
    # E-NAVI dataset
    if args.dataset.lower() == 'e-navi':
        return visualization.EnaviVisualizerEvents
    # DSEC dataset
...
def test(args):
    ...
    if args.dataset.lower()=='e-navi':
        if args.type.lower()=='standard':
            config_path = 'config/enavi_standard.json'
        else:
            raise Exception('Sorry, this is not implemented yet, please choose --type standard')
    elif args.dataset.lower()=='dsec':
    ...
    # Load Checkpoint
    checkpoint = torch.load(config['test']['checkpoint'], map_location=torch.device('cpu'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', default="e-navi", type=str, help="Which dataset to use: ([e-navi]/dsec/mvsec)")
    parser.add_argument('-f', '--frequency', default=20, type=int, help="Evaluation frequency of MVSEC dataset ([20]/45) Hz")
    parser.add_argument('-t', '--type', default='standard', type=str, help="Evaluation type ([standard]/warm_start)")
