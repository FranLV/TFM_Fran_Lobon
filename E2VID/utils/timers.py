...
def print_timing_info():
    ...
    for timer_name, timing_values in [*cuda_timers.items(), *timers.items()]:
        ...
        if timer_name == "Inference":
            timing_values = timing_values[1:]
        timing_value = np.mean(timing_values)
        if timing_value < 1000.0:
            print('{}: {:.2f} ms. Min: {:.2f} ms. Max: {:.2f} ms'.format(timer_name, timing_value, timing_values.min(),timing_values.max()))
        else:
            print('{}: {:.2f} s. Min: {:.2f} s. Max: {:.2f} s'.format(timer_name, timing_value / 1000.0, timing_values.min() / 1000.0,timing_values.max() / 1000.0))
