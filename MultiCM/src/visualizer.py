...
class Visualizer:
    ...
    def __init__(self, image_shape: tuple, show=False, save=False, save_dir=None) -> None:
        ...
        self.base_save_dir = self.save_dir
    ...
    def get_filename_from_prefix(
        ...
        if prefix is None or prefix == "":
            ...
            count_files = len([name for name in os.listdir(self.save_dir) if os.path.isfile(os.path.join(self.save_dir, name))])

            count_files += 1

            file_name = os.path.join(
                self.save_dir, f"{self.default_prefix}{count_files}.{file_format}"
            )
            self.default_save_count = count_files
        else:
            try:
    ...
    def visualize_image(self, image: Any, file_prefix: Optional[str] = None) -> Image.Image:
        ...
        self.update_save_dir(os.path.join(self.base_save_dir, "events"))
        self._show_or_save_image(image, file_prefix)
        self.save_dir_to_base()
        return image
    ...
    def visualize_optical_flow(
        ...
        self.update_save_dir(os.path.join(self.base_save_dir, "flow"))
        self._show_or_save_image(image, file_prefix)
    ...
    def visualize_overlay_optical_flow_on_event(
        ...
        self.update_save_dir(os.path.join(self.base_save_dir, "flow/dense"))
    ...
    def visualize_optical_flow_on_event_mask(
        ...
        self._show, self._save = _show, _save
        self.update_save_dir(os.path.join(self.base_save_dir, "flow/sparse"))
