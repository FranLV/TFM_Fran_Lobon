...
from .scipy_autograd.torch_wrapper import TorchWrapper
...
class PyramidalPatchContrastMaximization(PatchContrastMaximization):
    ...
    def run_scipy(self, events: np.ndarray, coarser_motion: dict) -> scipy.optimize.OptimizeResult:
        ...
        if (
            ...
            if self.slv_config["patch"]["initialize"] == "random":
                ...
                best_guess = self.initialize_guess_from_whole_image(events.clone().detach().cpu().numpy())
                ...
                best_guess = self.initialize_guess_from_patch(
                    events.clone().detach().cpu().numpy(), patch_index=self.n_patch // 2 - 1
                )
                if isinstance(best_guess, torch.Tensor):
                    best_guess = best_guess.detach().cpu().numpy()
                motion0 = np.tile(best_guess[None], (self.n_patch, 1)).T.reshape(-1)
        ...
        # result = TorchWrapper(lambda x: self.objective_scipy(x, events, coarser_motion), precision="float32", hvp_type=None, device=self._device)

        
        # result = result.get_output(result)
    ...
    def visualize_one_batch_warp(self, events: np.ndarray, warp: Optional[dict] = None):
        ...
        self.visualizer.visualize_image(clipped_iwe) # IWE IMAGE
        if warp is not None:
            self.visualizer.visualize_optical_flow_on_event_mask(flow, events) # SPARSE FLOW IMAGE
            self.visualizer.visualize_overlay_optical_flow_on_event(flow, clipped_iwe) # DENSE FLOW IMAGE
