...
if __name__ == "__main__":
    ...
    # Not evaluation - all frame optimization
    if config["is_dnn"]:
        e = "DNN code is not published."
        logger.error(e)
        raise NotImplementedError(e)
        ...
        if data_config["dataset"] == "E-NAVI":
            
            with open(out_config["output_dir"] + "/time_file.csv", "w+") as time_file:

                time_file.write("Frame,Time,Events\n")

                for idx in range(loader.get_max_image_indices()):
                    batch: np.ndarray = loader.get_events(idx)
                    batch[..., 2] -= np.min(batch[..., 2])
                    events_count = batch.shape[0] 

                    # solv.visualize_one_batch_warp(batch)
                    t_total = time.perf_counter()
                    best_motion: np.ndarray = solv.optimize(batch)
                    t_total = time.perf_counter() - t_total
                    time_file.write("{},{},{}\n".format(idx,t_total,events_count))
                    # time_file.flush()
